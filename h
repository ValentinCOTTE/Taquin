...
char* ecrire(char* chaine,char c){
    char* nouvelle=malloc(sizeof(char)*1000);
    nouvelle[0]=c;
    int i = 0;
...
void afficherEtape(char* a ,struct Etat etat)
{
    printf("\n");
    int i =longueurChaine(a);
    for(i;i>=0;i--)
    {
...
int main()
{
    ...
    //Saisie manuel
   /* struct Etat etat;
    etat.tab[0][0]=8;etat.tab[0][1]=1;etat.tab[0][2]=5;
    etat.tab[1][0]=4;etat.tab[1][1]=0;etat.tab[1][2]=3;
    etat.tab[2][0]=7;etat.tab[2][1]=6;etat.tab[2][2]=2;
    etat.ligne=0;
    etat.colonne=0;
...
*/
    ...
    afficherEtape(solution(etat),etat);
